...
import yaml
from ocr_database_handler import OCRDatabase  # Ensure correct import
from PIL import Image, ImageTk  # Tkinter Image handling

class VehicleNumberPlateProcessor:
    def __init__(self, video_path, target_fps=10, buffer_size=50):
        ...
        self.frame_queue = queue.Queue(maxsize=buffer_size)  # Queue for processed frames
    ...
    def start_processing(self):
        ...
        self.frame_buffer.start_reading()
        self.video_path = self.video_path if self.video_path != 0 else 0
    ...
    def stop_processing(self):
        ...
        if not self.is_processing:
            ...
            return
        self.is_processing = False
        self.frame_buffer.stop_reading()
    ...
    def get_processed_frame(self):
        """Retrieve a processed frame from the queue."""
        if not self.frame_queue.empty():
            return self.frame_queue.get()
        return None

    def _process_frames(self):
        """Process frames while checking for pause/resume conditions."""
        video_name = Path(self.video_path).stem if self.video_path != 0 else "webcam"
        video_id = self.db.insert_video(video_name)
        while self.is_processing:
            self.pause_event.wait()  # Pause processing if event is cleared
            frame = self.frame_buffer.get_frame()
            if frame is None:
                time.sleep(0.01)  # Avoid CPU overuse
                continue
            self._process_frame(frame, video_id)

    def _process_frame(self, frame, video_id):
            """Detect number plates, check blacklist, annotate, and store results."""

            # Optional: Clear previous data (use only if you want to overwrite every frame)
            ...
            for box in boxes:
                ...
                # Draw bounding box for all detected number plates (consistent color)
                cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 0), 3)  # Blue for all plates
                ...
                try:
                    ...
                    if combined_text:
                        ...
                        if formatted_plate:
                            # Perform OCR annotation only for valid Indian number plates
                            ...
                            try:
                                if self.db.is_plate_blacklisted(formatted_plate):
                                    ...
                                    for i, line in enumerate(alert_message.split("\n")):
                                        self._annotate_frame_with_text(
                                            frame, line, x1, y1 + i * 30, is_blacklisted=True
                                        )
                                        # Log blacklisted plate to separate YAML
                                    ...
                                    # Annotate non-blacklisted valid plate
                                    self._annotate_frame_with_text(
                                        frame, formatted_plate, x1, y1, is_blacklisted=False
                                    )
                            except Exception as e:
                                print(f"Blacklist check failed: {e}")

                            # Insert into DB
                            print(f"Inserting into DB: {formatted_plate}, Blacklisted: {is_blacklistNumberPlate}")
                            self.io_worker.queue.put({
                                'type': 'db_insert',
                                'func': self.db.insert_or_update_ocr_result,
                                'args': (video_id, base64_image, formatted_plate, is_blacklistNumberPlate)
                            })

                            # General YAML logging
                            self.io_worker.queue.put({
                                'type': 'write_yaml',
                                'data': {
                                    'numberPlate': formatted_plate,
                            ...
                            print(f"Invalid plate format: {combined_text}")
                    else:
            ...
            try:
                ...
                print(f"Error queuing processed frame: {e}")
  


    def _get_blacklisted_plate(self, plate_number):
        if self.db.is_blacklisted(plate_number):
            return plate_number
        return None
    ...
    def _extract_text_from_ocr(self, ocr_results):
        if not ocr_results:
            return None
        combined_text = " ".join([result[1][0].strip() for result in ocr_results[0]])
    ...
    def _is_valid_indian_vehicle_number_plate(self, text):
        ...
        if not text or not isinstance(text, str):
            return None
        text = ''.join(corrections.get(char, char) for char in text)
        sanitized_text = re.sub(r"[^A-Za-z0-9]", "", text).upper()
        pattern = r"^[A-Z]{2}[0-9]{1,2}[A-Z]{1,3}[0-9]{4}$"
        return sanitized_text if re.match(pattern, sanitized_text) else None

    def _handle_plate_data(self, plate, cropped_image):
        pass
    ...
    def _annotate_frame_with_text(self, frame, text, x1, y1, is_blacklisted=False):
        ...
        cv2.rectangle(frame,
                      ...
                      (text_x + text_width + padding, text_y),
                      color, -1)
        cv2.putText(frame, text, (text_x, text_y - padding),
                    cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), font_thickness)



class FrameBuffer:
    def __init__(self, video_path, buffer_size=50):
        self.video_path = video_path
    ...
    ## Add skip_rate as parameter (default: skip 1 of every 2 frames)
    def _read_frames(self, skip_rate=2):  # Add skip_rate as parameter (default: skip 1 of every 2 frames)
        ...
        frame_index = 0  # Counter for skipping

        while self.is_reading and self.cap.isOpened():
        ...
        self.cap.release()
        print("Video/Camera reading completed.")
...
class IOWorker(threading.Thread):
    def __init__(self, yaml_path, blacklist_yaml_path="BlackListNumberPlate_data.yaml"):
        ...
        self.blacklist_lock = threading.Lock()  # For thread safety
    ...
    def run(self):
        while True:
            ...
            try:
                task = self.queue.get(timeout=0.1)  # Wait max 0.1s
            except queue.Empty:
                continue
    ...
    def _cleanup_expired_blacklist_entries(self):
        ...
        with self.blacklist_lock:
            original = len(self.blacklist_data)
            self.blacklist_data = [
                entry for entry in self.blacklist_data
                if current_time - entry.get('timestamp', 0) < 3
            ]
            if len(self.blacklist_data) != original:
                self._write_blacklist_yaml()
